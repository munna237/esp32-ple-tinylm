...
// Set to 1 once a GMT020-02-7P (2.0" 240x320 ST7789) is wired up â€” see display.h.
...
static void emit(int tok) {
    ...
    // NOTE: VOCAB_OFF must be defined in vocab.h for this to compile!
...
static int8_t *head_actq = NULL;    // quantized activation, dynamically allocated to avoid overflow
...
void setup() {
  ...
  Serial.printf("temp_start: %.1f\n", temperatureRead());
    ...
    model.tok_emb.rows = c->vocab < VOCAB_N ? c->vocab : VOCAB_N;
  ...
  t_start = esp_timer_get_time();
    ...
    for (int step = 0; step < N_GENERATE && pos < model.c.seq_len; step++) {
        ...
        // Bound the loop by both VOCAB_N and V to prevent out-of-bounds read on s.logits
        int limit = (VOCAB_N < V) ? VOCAB_N : V;
        ...
        if ((step & (WDT_EVERY - 1)) == 0) delay(0);  // feed the task WDT ~every 8 tokens (~1.1s), near-free
    }
